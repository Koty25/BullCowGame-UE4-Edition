...
void UTerminal::PrintGuess(const FString& Line)
{
	FString Input = Line;
	FString Left, Right;
	while (Input.Split(TEXT("\n"), &Left, &Right))
	{
		BufferGuess.Emplace(Left);
		Input = Right;
	} 
	BufferGuess.Emplace(Input);
	UpdateGuess();
}
...
FString UTerminal::GetScreenGuess()
{
	TArray<FString> FullTerminal = BufferGuess;
...
void UTerminal::UpdateGuess()
{
	GuessUpdated.Broadcast(GetScreenGuess());
}