...
protected:
	...
	void PrintGuess(const TCHAR* Line) const;
	void PrintGuess(const FString& Line) const;
	...
	template<SIZE_T N, typename ...Types>
	void PrintGuess(const TCHAR (&Fmt)[N], Types... Args) const
	{
		PrintGuess(FString::Printf(Fmt, Args...));
	}
